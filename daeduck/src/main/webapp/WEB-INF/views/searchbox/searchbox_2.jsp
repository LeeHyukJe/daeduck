...
<!--  검색창 -->
	<form action="/searchAll" method="get" id="search" >
		...
		<div class="searchbox">
			...
			<c:if test="${requestScope['javax.servlet.forward.request_uri'] eq '/colquality'}">
				<button type="submit" class="srch_submit" value="검색" onClick="javascript:doQualitySearch('${value.colflag}','${value.collection}')">검색</button>
			</c:if>

			<c:if test="${requestScope['javax.servlet.forward.request_uri'] != '/colquality'}">
				<button type="submit" class="srch_submit" value="검색" onClick="javascript:doSearch('${value.collection}')">검색</button>
			</c:if>
			...
			<button type="button" value="상세검색" onClick="javascript:showDetailBox()">상세검색</button>
		</div>
	...
	<c:if test="${requestScope['javax.servlet.forward.request_uri'] eq '/search' || requestScope['javax.servlet.forward.request_uri'] eq '/searchAll'}">
